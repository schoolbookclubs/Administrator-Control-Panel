...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post('https://school-book-clubs-backend.vercel.app/api/GeneralSupervisor/login', {
        email,
        password
      });
      ...
      setModalMessage(error.response?.data?.message || 'حدث خطأ أثناء تسجيل الدخول');
      setShowModal(true);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="login-container">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered className="custom-modal">
        <Modal.Header closeButton>
          <Modal.Title>{modalVariant === 'success' ? 'نجاح' : 'خطأ'}</Modal.Title>
        </Modal.Header>
